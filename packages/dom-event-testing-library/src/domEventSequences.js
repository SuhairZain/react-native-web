...
export function focus(target, payload) {
  const dispatch = arg => target.dispatchEvent(arg);
  dispatch(domEvents.focusout(payload));
  dispatch(domEvents.focusin({ ...payload, relatedTarget: target }));
  dispatch(domEvents.blur(payload));
  dispatch(domEvents.focus({ ...payload, relatedTarget: target }));
}
