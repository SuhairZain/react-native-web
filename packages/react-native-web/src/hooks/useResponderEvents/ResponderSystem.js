...
const emptyResponder = { id: null, node: null, path: null };
...
/**
 * Instance that should respond to pointer interactions,
...
function getResponderCallbacks(id: ResponderId): ResponderCallbacks | Object {
...
function eventListener(domEvent: any) {
  ...
  if (isStartEvent || isMoveEvent || (isScrollEvent && trackedTouchCount > 0)) {
    ...
    if (currentResponder.path != null && eventPaths.path != null) {
      const lowestCommonAncestor = getLowestCommonAncestor(currentResponder.path, eventPaths.path);
      const indexOfLowestCommonAncestor = eventPaths.path.indexOf(lowestCommonAncestor);
      // Skip the current responder so it doesn't receive unexpected "shouldSet" events
      const index =
        indexOfLowestCommonAncestor + (lowestCommonAncestor === currentResponder.id ? 1 : 0);
      eventPaths = {
        path: eventPaths.path.slice(index),
 ...
 * relevant "shouldSet" functions for the given event type.
 * TODO: calling stopPropagation should kill the responder bubble/capture logic
 */
function findWantsResponder(eventPaths, domEvent, responderEvent) {
  const shouldSetCallbacks = shouldSetResponderEvents[(domEvent.type: any)]; // for Flow

  if (shouldSetCallbacks != null) {
    const { path, nodePath } = eventPaths;
    ...
    const check = function(id, node, callbackName) {
      ...
      const shouldSetCallback = callbacks[callbackName];
...
function attemptTransfer(responderEvent: ResponderEvent, wantsResponder: ActiveResponderInstance) {
  ...
  // negotiate with current responder
    ...
    const { onResponderTerminate, onResponderTerminationRequest } = getResponderCallbacks(
      currentId
    );
    const allowTransfer =
 ...
 */
export function addNode(id: ResponderId, node: any, callbacks: ResponderCallbacks) {
