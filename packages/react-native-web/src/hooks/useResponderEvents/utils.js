 ...
 */
export function getResponderPaths(domEvent: any): {| nodePath: Array<any>, path: Array<number> |} {
  const nodePath = [];
  const path = [];
...
/**
 * The browser fires a lot of 'selectionchange' events. This can be used to ignore
 * meaningless selections.
 */
export function hasValidSelection(domEvent: any) {
  if (domEvent.type === 'selectionchange') {
    const selection = window.getSelection().toString();
    const anchorNode = window.getSelection().anchorNode;
    const isTextNode = anchorNode && anchorNode.nodeType === window.Node.TEXT_NODE;
...
/**
 * Events are only valid if the primary button was used without modifier keys.
 */
export function isPrimaryPointerDown(domEvent: any): boolean {
  const { altKey, button, buttons, ctrlKey, metaKey, shiftKey, type } = domEvent;
